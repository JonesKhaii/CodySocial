...
class AuthController extends Controller
{
    ...
    public function processLogin(Request $request)
    {
        $credentials = $request->validate([
            ...
            'role' => 'required|string|in:user,doctor', // Kiểm tra role hợp lệ
        ]);
        ...
        if ($credentials['role'] == 'user') {
            ...
            if ($user && Hash::check($credentials['password'], $user->password)) {
                Auth::shouldUse('web'); // Đặt guard mặc định là web
                Auth::login($user);
            ...
            if ($doctor && Hash::check($credentials['password'], $doctor->password)) {
                Auth::shouldUse('doctor'); // Đặt guard mặc định là doctor
                Auth::guard('doctor')->login($doctor);
                session(['role' => 'doctor']); // Lưu role vào session
                session()->save();
                return redirect()->route('doctor.profile')->with('success', 'Đăng nhập thành công!');
            }
        }

        return back()->withErrors(['phone' => 'Thông tin đăng nhập không đúng'])->withInput();
    }
