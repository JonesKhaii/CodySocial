...
class AuthController extends Controller
{
    ...
    //         'role' => 'required|string|in:user,doctor',  // Kiểm tra vai trò
    //     ]);
    ...
    //             Log::info('Đăng nhập thành công với vai trò Người Dùng', [
    //                 'user_id' => $user->id,
    //                 'phone' => $user->phone,
    //             ]);
    ...
    //             Log::info('Đăng nhập thành công với vai trò Bác Sĩ', [
    //                 'doctor_id' => $doctor->id,
    //                 'phone' => $doctor->phone,
    //             ]);
    //             return redirect()->intended(route('home'));  // Điều hướng đến trang tổng quan bác sĩ
    //         }
    //     }
    ...
    public function processLogin(Request $request)
    {
        ...
        if ($credentials['role'] == 'user') {
            ...
            if ($doctor && Hash::check($credentials['password'], $doctor->password)) {
                Auth::guard('doctor')->login($doctor);  // Đăng nhập bác sĩ
                session(['role' => 'doctor']);
                return redirect()->intended(route('home'));
            }
        }
