...
class HomeController extends Controller
{
    public function index()
    {
        ...
        $posts = Post::select('id', 'title', 'slug', 'summary', 'photo', 'post_cat_id', 'post_tag_id', 'added_by', 'created_at')
            ->where('status', 'active')
            ->with([
                'cat_infor:id,title', // Lấy tiêu đề danh mục bài viết
                'tag_infor:id,title', // Lấy tên các tag liên kết với bài viết
                'author_infor:id,name', // Lấy thông tin tác giả (name)
            ])
            ->latest()
