...
@section('main-content')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            let categorySelect = document.getElementById('post_cat_id');

            if (categorySelect) {
                let options = categorySelect.options;

                console.log("Tổng số danh mục:", options.length);

                if (options.length > 1) { // Vì option đầu tiên là "Chọn danh mục"
                    for (let i = 1; i < options.length; i++) { // Bỏ qua option đầu tiên
                        console.log("Danh mục ID:", options[i].value, " - Tên:", options[i].text);
                    }
                } else {
                    console.warn("⚠ Không có danh mục nào được load vào select!");
                }
            } else {
                console.error("❌ Không tìm thấy thẻ select #post_cat_id!");
            }
        });
    </script>
