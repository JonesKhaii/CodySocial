...
@section('main-content')
    ...
    <div class="container py-4">
        ...
        <ul class="nav nav-pills mb-4">
            @foreach (['Chờ duyệt' => 'pending', 'Sắp tới' => 'confirmed', 'Hoàn thành' => 'completed', 'Đã Huỷ' => 'canceled'] as $label => $status)
                <li class="nav-item">
                    <a class="nav-link {{ $loop->first ? 'active' : '' }}" data-bs-toggle="pill"
                        href="#{{ $status }}">
                        <i
                            class="fas fa-{{ $status == 'pending' ? 'clock' : ($status == 'confirmed' ? 'check-circle' : ($status == 'completed' ? 'check-double' : 'times-circle')) }} me-2"></i>{{ $label }}
                        @if ($appointments->where('status', $status)->count() > 0)
                            <span
                                class="badge bg-warning ms-2">{{ $appointments->where('status', $status)->count() }}</span>
                        @endif
                    </a>
                </li>
            @endforeach
